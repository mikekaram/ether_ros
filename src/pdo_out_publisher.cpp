...
void PDOOutPublisher::pdo_raw_callback(const ighm_ros::PDORaw::ConstPtr &pdo_raw)
{
    ...
    for (int i = 0; i < master_info.slave_count; i++)
    {
        ...
        pdo_out.sync = process_input_sint8(data_ptr, 1);
        pdo_out.desired_x_value = process_input_sint32(data_ptr, 2);
        pdo_out.filter_bandwidth = process_input_uint16(data_ptr, 6);
        pdo_out.desired_y_value = process_input_sint32(data_ptr, 8);
        pdo_out.kp_100_knee = process_input_sint16(data_ptr, 12);
        pdo_out.kd_1000_knee = process_input_sint16(data_ptr, 14);
        pdo_out.ki_100_knee = process_input_sint16(data_ptr, 16);
        pdo_out.kp_100_hip = process_input_sint16(data_ptr, 18);
        pdo_out.kd_1000_hip = process_input_sint16(data_ptr, 20);
        pdo_out.ki_100_hip = process_input_sint16(data_ptr, 22);
        pdo_out.x_cntr_traj1000 = process_input_sint16(data_ptr, 24);
        pdo_out.y_cntr_traj1000 = process_input_sint16(data_ptr, 26);
        pdo_out.a_ellipse100 = process_input_sint16(data_ptr, 28);
        pdo_out.b_ellipse100 = process_input_sint16(data_ptr, 30);
        pdo_out.traj_freq100 = process_input_sint16(data_ptr, 32);
        pdo_out.phase_deg = process_input_sint16(data_ptr, 34);
        pdo_out.flatness_param100 = process_input_sint16(data_ptr, 36);
